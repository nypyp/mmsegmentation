...
class crackformer(nn.Module):
    ...
    def init_weights(self):
        pretrain_dir = "/home/nypyp/code/mmsegmentation/weights/crack537.pth"
        print("Loaded static_dict form local...,from:",pretrain_dir)
        self.load_state_dict(torch.load(pretrain_dir))

if __name__ == '__main__':
    # inp = torch.randn(1, 3, 512, 512)
    # inp = torch.load('/home/nypyp/code/mmsegmentation/weights/tensor_var.pt') 
    # # inp = inp[0].byte()
    # print(inp[0].shape)
    ...
    #     interpolated_tensor = torch.nn.functional.interpolate(tensor, size=(512,512), mode='bilinear')
    #     interpolated_tensors.append(interpolated_tensor)
    # inp = torch.stack(interpolated_tensors,dim=0)
    # print(inp.shape)
    inp = torch.load('/home/nypyp/code/mmsegmentation/weights/tensor_lossinput.pt')
    outmm = torch.load('/home/nypyp/code/mmsegmentation/weights/tensor_backboneout.pt')
    outmod = torch.load('/home/nypyp/code/mmsegmentation/weights/tensor_modelout.pt')
    model = crackformer().cuda()
    # out=model(inp)
    outmm_f = model.final(outmm.float())
    outmod_f = model.final(outmod.float().cuda())
    # torch.save(out,'/home/nypyp/code/mmsegmentation/weights/tensor_modelout.pt')
    # out=model.final(out)
    # print(model)
